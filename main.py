...
# -*- coding: utf-8 -*-
"""
TPMB2 - Classic Windows UI (Segoe UI 9, winnative/classic) + per-bot proxy + Update button
This replaces the main entry to default minimalist look
"""
...
class TPMB2:
    ...
    def _style(self):
        style = ttk.Style()
        for theme in ('winnative', 'classic', 'vista'):  # try in this order
            try:
    ...
    def _build(self):
        ...
        top.pack(fill=tk.X, padx=10, pady=(8,6))
        ...
        self.lbl_status.pack(side=tk.LEFT, padx=(4,16))
        ttk.Label(top, text='Groups:').pack(side=tk.LEFT)
        ttk.Label(top, textvariable=self.var_groups).pack(side=tk.LEFT, padx=(4,16))
        ttk.Label(top, text='License:').pack(side=tk.LEFT)
        self.lbl_license = ttk.Label(top, textvariable=self.var_license)
        self.lbl_license.pack(side=tk.LEFT, padx=(4,16))
        
        # Right controls
        right = ttk.Frame(top)
        right.pack(side=tk.RIGHT)
        self.btn_start = ttk.Button(right, text='Start', command=self._start)
        self.btn_restart = ttk.Button(right, text='Restart', command=self._restart, state=tk.DISABLED)
        self.btn_stop = ttk.Button(right, text='Stop', command=self._stop, state=tk.DISABLED)
        self.btn_update = ttk.Button(right, text='Update', command=self._update)
        for b in (self.btn_start, self.btn_restart, self.btn_stop, self.btn_update):
    ...
    def _tab_config(self, p):
        ...
        ttk.Label(p, text='Bot Token:').grid(row=0, column=0, sticky='w', pady=4)
        self.var_token = tk.StringVar()
        ttk.Entry(p, textvariable=self.var_token, show='*', width=48).grid(row=0, column=1, sticky='ew', padx=8)
        tbtn = ttk.Frame(p); tbtn.grid(row=0, column=2)
        ttk.Button(tbtn, text='Save', command=self._save_token).pack(side=tk.LEFT)
        ttk.Button(tbtn, text='Test', command=self._test_conn).pack(side=tk.LEFT, padx=6)
        
        lf = ttk.LabelFrame(p, text='Message Templates')
        lf.grid(row=1, column=0, columnspan=3, sticky='ew', pady=(8,0))
        lf.columnconfigure(1, weight=1)
        ttk.Label(lf, text='Active:').grid(row=0, column=0, sticky='w')
        self.var_tpl = tk.StringVar(value=self.cfg.get_active_template_key())
        self.cb_tpl = ttk.Combobox(lf, textvariable=self.var_tpl, state='readonly')
        self.cb_tpl['values'] = self.cfg.list_templates()
        self.cb_tpl.grid(row=0, column=1, sticky='ew', padx=8)
        ttk.Button(lf, text='Set', command=self._tpl_set).grid(row=0, column=2)
        ttk.Button(lf, text='New', command=self._tpl_new).grid(row=0, column=3, padx=6)
        ttk.Button(lf, text='Delete', command=self._tpl_del).grid(row=0, column=4)
        ...
        ttk.Button(p, text='Save Message', command=self._msg_save).grid(row=3, column=1, sticky='w', padx=8, pady=6)

    def _tab_bot(self, p):
        p.columnconfigure(1, weight=1)
        lf_a = ttk.LabelFrame(p, text='Active Bot')
        lf_a.grid(row=0, column=0, columnspan=3, sticky='ew', pady=(0,6))
        ...
        ttk.Button(ab, text='Set Active', command=self._bot_set_active).pack(side=tk.LEFT)
        ...
        lf_l = ttk.LabelFrame(p, text='License')
        lf_l.grid(row=1, column=0, columnspan=3, sticky='ew', pady=(0,6))
        lf_l.columnconfigure(1, weight=1)
        ttk.Label(lf_l, text='Key:').grid(row=0, column=0, sticky='w', pady=4)
        self.var_key = tk.StringVar()
        ttk.Entry(lf_l, textvariable=self.var_key, width=54).grid(row=0, column=1, sticky='ew', padx=8)
        ...
        lf_p = ttk.LabelFrame(p, text='SOCKS5 Proxy (Per-Bot)')
        lf_p.grid(row=2, column=0, columnspan=3, sticky='ew')
        ...
        ttk.Button(pb, text='Save Proxy', command=self._proxy_save).pack(side=tk.LEFT)
        ttk.Button(pb, text='Test Connection', command=self._proxy_test).pack(side=tk.LEFT, padx=6)
        self._proxy_load()
        
        lf_i = ttk.LabelFrame(p, text='Bot Information')
        lf_i.grid(row=3, column=0, columnspan=3, sticky='nsew', pady=(6,0))
    ...
    def _tab_logs(self, p):
        ...
        ttk.Button(ctrl, text='Refresh', command=self._logs_refresh).pack(side=tk.LEFT)
    ...
    def _tab_groups(self, p):
        ...
        ttk.Button(ctrl, text='Add', command=self._grp_add).pack(side=tk.LEFT, padx=6)
